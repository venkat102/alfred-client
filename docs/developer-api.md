...
| `changeset` | Final changeset for approval. Includes `changes`, `dry_run`, `result_text`. The `dry_run` object now carries a `status` field distinguishing `ok` (validated, safe), `invalid` (validated, has content issues), `infra_error` (validation DID NOT run - MCP down, permission denied, Frappe internal error), and `skipped` (no bench attached). UI MUST disable the Approve button on `infra_error` - approving there would deploy a changeset that was never actually validated. |
...
| `plan_doc` | Plan document from Plan mode (Phase C). Fires when the orchestrator routes to `plan` mode. Payload: `{conversation, plan, mode: "plan"}` where `plan` is a `PlanDoc`-shaped dict (`{title, summary, steps, doctypes_touched, risks, open_questions, estimated_items, parse_failed, parse_failure_detail}`). Rendered as a structured panel (`PlanDocPanel.vue`) with Refine / Approve & Build buttons. Produced by a 3-agent crew (Requirement / Assessment / Architect) with a 15-call tool budget. No changeset, no DB writes. **When `parse_failed=true`, the UI MUST disable the Approve / Build button** - the plan is an error stub whose `steps` are placeholder text, not a real plan; `parse_failure_detail` carries a human-readable reason plus up to 500 chars of the raw agent output for debugging. A successful plan has `parse_failed=false` and `parse_failure_detail=null`. |
...
| `info` | Non-blocking notice from the server. Carries `{message, code, response_to?}`. The Frappe-side event map forwards these to browsers as `frappe.realtime` event `alfred_info`; `AlfredChatApp.vue` renders via `frappe.show_alert({message, indicator})` - orange for MEMORY_SAVE_FAILED, blue for CLARIFIER_LATE_RESPONSE + unknown future codes. Codes emitted today: `CLARIFIER_LATE_RESPONSE` - user answered a clarifier question after its 900s Future timed out; pipeline proceeded without them and this ack lets the UI surface "your answer arrived too late, send as a new prompt if you want to apply it". `MEMORY_SAVE_FAILED` - conversation memory could not be persisted to Redis at the end of a turn (chat / insights / plan / dev); the primary output (changeset / reply) still reached the user, but follow-up prompts in this conversation may not recall this turn's context. |
